from LayeredNeuralAtlas.implicit_neural_networks import IMLP
import torch
import torch.optim as optim
import numpy as np

from LayeredNeuralAtlas.evaluate import evaluate_model
from datetime import datetime
from LayeredNeuralAtlas.loss_utils import get_gradient_loss, get_rigidity_loss, \
...
'''
이 파일은 다음과 같은 방식으로 진행된다.
1. config 파일에서 하이퍼파라미터를 가져온다.
2. mapping, atlas, alpha 모델을 생성하고 
   mapping 모델들은 pretrain한다. (이전에 학습한 모델을 불러오는 경우면 100회 pretrain 하는 과정은 생략한다.
3. 학습을 진행한다.
   이 때 loss function들을 정의하고 backpropagation을 가한다.
   그리고 logging 함수를 통해 loss값들을 모두 출력한다.
   
   그리고 일정 간격마다 비디오를 렌더링하고 평가한다. (evaluate.py에서 정의한 함수들을 쓴다.)
'''
...
def main(config):
    ...
    results_folder_name = config[
        "results_folder_name"]  # the folder (under the code's folder where the experiments will be saved.
    add_to_experiment_folder_name = config[
        "add_to_experiment_folder_name"]  # for each experiment folder (saved inside "results_folder_name") add this string
    ...
    optical_flows_mask, video_frames, optical_flows_reverse_mask, mask_frames, \
    video_frames_dx, video_frames_dy, optical_flows_reverse, optical_flows = load_input_data(
        resy, resx, maximum_number_of_frames, data_folder,
        True, True, vid_root, vid_name)
    number_of_frames=video_frames.shape[3]
    # save a video showing the masked part of the forward optical flows
    save_mask_flow(optical_flows_mask, video_frames, results_folder)

    # 모델은 모두 MLP이다.
    # mapping1 (foreground), mapping2 (background)는 (x,y,t) -> (u,v) 변환을 한다.
    # model_F_atlas는 (u,v) -> (R,G,B) 변환을 한다.
    # model_alpha는 (x,y,t) -> alpha(투명도) 변환을 한다.
    ...
    if not load_checkpoint:
        ...
        '''
        논문의 appendix A 두번째 문단을 읽을것.
        uv로 mapping하는 foreground, background MLP들을 pretrain한다. 
        identity mapping이 되도록 ((x,y,t) -> (x,y)) 100 iteration 동안 학습한다.
        이렇게 하면 mapping이 뒤집혀 나오지 않도록 방지한다.
        '''

        if pretrain_mapping1:
            model_F_mapping1 = pre_train_mapping(model_F_mapping1, number_of_frames,
                                                 uv_mapping_scale, resx=resx, resy=resy,
                                                 larger_dim=larger_dim, device=device,
                                                 pretrain_iters=pretrain_iter_number)
        if pretrain_mapping2:
            model_F_mapping2 = pre_train_mapping(model_F_mapping2, number_of_frames,
                                                 uv_mapping_scale, resx=resx, resy=resy,
                                                 larger_dim=larger_dim, device=device,
                                                 pretrain_iters=pretrain_iter_number)
    else:
    ...
    for i in range(start_iteration, iters_num):
        ...
        # (for the BCE loss that uses log(alpha), log(1-alpha) below)
        alpha = alpha * 0.99
        alpha = alpha + 0.001

        # Sample atlas values. Foreground colors are sampled from [0, 1]x[0, 1]
        # and background colors are sampled from [-1, 0]x[-1, 0]
        # Note that the original [u, v] coordinates are in [-1, 1]x[-1, 1] for both networks
        rgb_output1 = (model_F_atlas(uv_foreground1 * 0.5 + 0.5) + 1.0) * 0.5
        rgb_output2 = (model_F_atlas(uv_foreground2 * 0.5 - 0.5) + 1.0) * 0.5
        ...
        # training step 마다 이 모든 loss값들을 출력한다.
        ...
        try:
            ...
            if i % evaluate_every == 0 and i > start_iteration:
                evaluate_model(model_F_atlas, resx, resy, number_of_frames,
                               model_F_mapping1, model_F_mapping2, model_alpha,
                               video_frames, results_folder, i, mask_frames,
                               optimizer_all, writer, vid_name, derivative_amount,
                               uv_mapping_scale, optical_flows, optical_flows_mask,
                               device)
