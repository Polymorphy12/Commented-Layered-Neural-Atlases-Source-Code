...
def get_consistency_mask(optical_flow, optical_flow_reverse):
    ...
    mask_flow_reverse = compute_consistency(optical_flow_reverse.numpy(), optical_flow.numpy()) < 1.0
...
def load_input_data(resy, resx, maximum_number_of_frames, data_folder, use_mask_rcnn_bootstrapping,
                    filter_optical_flow, vid_root, vid_name):
...
def get_tuples(number_of_frames, video_frames):
    # video_frames_shape: (resy, resx, 3, num_frames)
...
identity mapping ((x,y,t) -> (x,y))를 100 iteration 동안 학습한다.
이렇게 하면 mapping이 뒤집혀 나오지 않도록 방지한다.
'''
def pre_train_mapping(model_F_mapping, frames_num, uv_mapping_scale, resx, resy,
                      larger_dim, device, pretrain_iters=100):
    ...
    for j in range(optical_flows_mask.shape[3]):
        ...
        for i in range(video_frames.shape[3]):
            ...
            cur_frame[
                ...
                torch.where(optical_flows_mask[:, :, i, j] == 0)[1], 0] = 1
            cur_frame[
                torch.where(optical_flows_mask[:, :, i, j] == 0)[0],
                torch.where(optical_flows_mask[:, :, i, j] == 0)[1], 1] = 0
            cur_frame[
                torch.where(optical_flows_mask[:, :, i, j] == 0)[0],
                torch.where(optical_flows_mask[:, :, i, j] == 0)[1], 2] = 0
