...
from LayeredNeuralAtlas.evaluate import get_high_res_texture, get_colors,get_mapping_area
import os
...
'''
1)  이 코드를 이해하려면 먼저 맨 아래의 
    if __name__ == "__main__":  부분을 봐야 한다.

    원본 비디오 이미지는 data_folder(frames_folder)을 args로 받아서 불러온다.
    이 비디오 이미지는 video_frames라는 변수로 저장한다.
    
    세 가지 종류의 파일을 받는다. 
    이 파일들은 파일전체, 또는 foreground나 background를 편집한 파일들이다.

    frame_edit_file = args.edit_frame_path
    -> 이 파일을 받을 경우 
       texture_edit_from_frame_edit 함수를 통해
       이 파일로부터 edit_im1, edit_im2를 생성한다.
       args.edit_frame_foreground를 받으면 foreground에만 편집을 가하고,
       args.edit_frame_background를 받으면 background에만 편집을 가한다.

    edit_tex1_file = args.edit_foreground_path
    edit_tex2_file = args.edit_background_path
    -> 이 두 파일을 받을 경우, foreground와 background를 따로 편집한 것이다.
       두 파일로부터 edit_im1, edit_im2를 생성한다.


2)  apply_edit 함수에 위에서 생성한 edit_im1, edit_im2 파일들을 넘겨준다.
    넘긴 파일들을 바탕으로 이미지 범위 내에 있는 픽셀 색깔을 추출해 
    프레임 별로
    논문의 eq. (15), (16)을 계산한다.
    
    이 eq.(15), (16)이 원본 비디오의 각 프레임(video_frames)에 편집한 것을 입혀주는 작업이다.
    
3)  여기까지 완료되면 비디오의 전체 프레임에 편집한 것이 반영되어 있다.
    편집한 비디오를 저장한다.
