...
def preprocess(args):
    ...
    for i in range(0,number_of_frames):
        ...
        # 이미지를 mask_rcnn 모델에 통과시킨 결과로 마스크를 얻는다.
        # class_name을 'anything'으로 설정했을 때는 mask_rcnn이 추출한 첫번째 물체를 마스크로 저장한다.
        # 그런 경우가 아니면, mask_rcnn이 추출한 물체들 중 원하는 물체들을 마스크로 저장한다.
        # 두 경우 모두 물체를 찾지 못했을 경우에는 0으로 체운 numpy_array를 저장한다.
